...
function openActivityDetail(detailKey) {
  ...
  if (detailTitle) detailTitle.textContent const walletPolishStylesheet = document.createElement("link");
walletPolishStylesheet.rel = "stylesheet";
walletPolishStylesheet.href = "wallet-polish.css?v=gift-popup-1";
document.head.appendChild(walletPolishStylesheet);
...
const premiumPolishStylesheet = document.createElement("link");
premiumPolishStylesheet.rel = "stylesheet";
premiumPolishStylesheet.href = "premium-polish.css?v=backend-loop-1";
document.head.appendChild(premiumPolishStylesheet);
...
const activityHeroDetail = document.querySelector("[data-activity-hero-detail]");
const activityHeroDots = Array.from(document.querySelectorAll("[data-activity-hero-dot]"));
const withdrawAmountInput = document.querySelector(".withdraw-card input");
const withdrawMethodSelect = document.querySelector(".withdraw-card select");
let walletBalanceValue = 0.5;
let activePromo = 0;
let activeActivityHero = 0;

const demoUserToken = "demo-user-token";
const userTokenKey = "ddslot777_user_token";

if (!localStorage.getItem(userTokenKey)) {
  localStorage.setItem(userTokenKey, demoUserToken);
}
...
async function apiRequest(path, options = {}) {
  const response = await fetch(`/api${path}`, {
    ...
    headers: {
      ...
      Authorization: `Bearer ${localStorage.getItem(userTokenKey) || demoUserToken}`,
      ...(options.headers || {}),
    },
  });
...
async function loadWalletState() {
  try {
    ...
    if (data.member) updateWalletBalance(Number(data.member.balance || 0));
  } catch (error) {
    if (rechargeNote) {
      rechargeNote.innerHTML = `Backend offline: <strong>demo balance stays local</strong>`;
    }
  }
}

async function submitDepositOrder(button, amount, label) {
  button.classList.add("is-processing");
  button.textContent = "Submitting...";
  button.disabled = true;
  if (rechargeNote) rechargeNote.innerHTML = `Creating deposit order: <strong>${label}</strong>`;

  try {
    const data = await apiRequest("/deposits", {
      method: "POST",
      body: JSON.stringify({ amount, method: "Demo Pay" }),
    });
    if (data.member) updateWalletBalance(Number(data.member.balance || walletBalanceValue));
    button.classList.remove("is-processing");
    button.classList.add("is-success");
    button.textContent = "Order Pending";
    if (rechargeNote) {
      rechargeNote.innerHTML = `Order <strong>${data.order.order}</strong> submitted. Admin confirmation will credit <strong>${formatUsd(data.order.amount + data.order.bonus)}</strong>.`;
    }
  } catch (error) {
    button.classList.remove("is-processing");
    button.textContent = "Deposit Now";
    if (rechargeNote) rechargeNote.innerHTML = `Deposit failed: <strong>${error.message}</strong>`;
  }
...
async function submitWithdrawalOrder(button) {
  const amount = parseAmountValue(withdrawAmountInput?.value || "0");
  const method = withdrawMethodSelect?.value || "Bank Transfer";
  if (amount <= 0) return;
  ...
  try {
    const data = await apiRequest("/withdrawals", {
      method: "POST",
      body: JSON.stringify({ amount, method, account: "Demo payout account" }),
    });
    if (data.member) updateWalletBalance(Number(data.member.balance || walletBalanceValue));
    button.classList.add("is-success");
    button.textContent = `Pending ${data.order.order}`;
  } catch (error) {
...
document.addEventListener("click", (event) => {
  ...
  const withdrawButton = event.target.closest("[data-withdraw-submit]");
  ...
  if (rechargeButton && rechargeNote) {
    ...
    submitDepositOrder(rechargeButton, depositValue, selectedAmount);
  }

  if (withdrawButton) {
    submitWithdrawalOrder(withdrawButton);
  }
});
